...
import { getDeliveryPersonnels } from 'apiSdk/delivery-personnels';
...
function DashboardPage() {
  ...
  const deliveryPersonnelStat = useSWR<PaginatedInterface<DeliveryPersonnelInterface>>(
    () => `/delivery-personnels`,
    () => getDeliveryPersonnels({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    },
    {
      ...
      result: deliveryPersonnelStat?.data?.totalCount,
      isLoading: deliveryPersonnelStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Orders',
